...
`;
  ...
  async createProcess(command: string, args: string[] = [], cwd: string = '/data/data/com.termux/files/usr'): Promise<string> {
  ...
  private setupSessionListeners(processId: string, session: { id: string; pid: number; fileDescriptor: number; isRunning: boolean }): void {
    ...
    TermuxCore.onSessionOutput(session.id, (data: string) => {
      ...
      if (process) {
        ...
        if (data.includes('Server running') || data.includes('localhost:3000')) {
      ...
      switch (command) {
        ...
        case 'npm':
          if (args.includes('--version')) {
            ...
            process.output.push('  ▲ Next.js 14.2.0');
            process.output.push('');
            process.output.push('   Creating an optimized production build ...');
            process.output.push('✓ Compiled successfully');
        ...
        case 'git':
          if (args.includes('status')) {
            process.output.push('On branch feature/terminal-preview-integration');
            process.output.push('Changes not staged for commit:');
            process.output.push('  modified:   src/components/NotionEditor.tsx');
            process.output.push('  modified:   src/services/TerminalService.ts');
            process.output.push('');
            process.output.push('no changes added to commit');
          } else if (args.includes('log')) {
            process.output.push('commit a1b2c3d4 (HEAD -> feature/terminal-preview-integration)');
            process.output.push('Author: user <user@mobile-dev-studio>');
            process.output.push('Date:   Sun Jul 13 12:00:00 2025');
            process.output.push('');
            process.output.push('    Add enhanced terminal simulation with development server integration');
          } else {
            process.output.push('git [--version] [--help] [-C <path>] [-c <name>=<value>]');
            process.output.push('           [--exec-path[=<path>]] [--html-path] [--man-path] [--info-path]');
            process.output.push('           [-p | --paginate | -P | --no-pager] [--no-replace-objects] [--bare]');
          }
          break;
        ...
        case 'curl':
          if (args.includes('localhost:3000')) {
            if (this.serverStatus.status === 'running') {
              ...
              process.output.push('<html><head><title>Notion Editor</title></head>');
              process.output.push('<body><h1>Mobile Dev Studio</h1></body></html>');
            } else {
              process.output.push('curl: (7) Failed to connect to localhost port 3000: Connection refused');
            }
          } else {
  ...
  private notifyServerStatus(status: 'stopped' | 'running' | 'error', url?: string): void {
  ...
  addEventListener(listener: (event: { type: string; data: any }) => void): void {
    this.listeners.push(listener);
  }

  removeEventListener(listener: (event: { type: string; data: any }) => void): void {
